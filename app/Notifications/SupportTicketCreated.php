...
class SupportTicketCreated extends Notification
{
    ...
    public function via($notifiable)
    {
        ...
        $channels=[];
        $NotificationSetting=NotificationSetting::where('alias','support_ticket_created')->first();
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
                    ...
                    ->line('You have raised a query, here is your ticket No - '.$this->ticket->id);
    }

    public function toSms($notifiable)
    {
        $Sms=Sms::where('title','Support Ticket Creation')->first();        
        if($Sms){
