<template>
  <div class="app-container">
    ...
    <el-dialog title="Debit Balance" width="60%"  top="30px" :visible.sync="dialogDebitBalanceVisible">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" icon="el-icon-finished" :loading="buttonLoading" @click="handleDebitBalance()">Debit Balance</el-button>
      </span>
    </el-dialog>
...
<script>
import { getDebitTransactions, createTransfer, debitBalance } from "@/api/admin/wallet";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getDebitTransactions(this.listQuery).then(response => {
    ...
    handleAddMemberGetBalance(){
       if(this.tempDebitBalance.member_id){
        getMemberBalance(this.tempDebitBalance.member_id).then((response) => {
          this.tempDebitBalance.member_name=response.data.name;
          this.tempDebitBalance.balance=response.data.member.wallet_balance;    
        }).catch((error)=>{
    ...
    handleDebitBalance(){
      this.$refs["formDebitBalance"].validate(valid => {
        ...
        if (valid) {
          ...
          debitBalance(this.tempDebitBalance).then((response) => {
            this.getList();
