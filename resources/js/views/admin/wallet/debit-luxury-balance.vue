<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      >Debit Luxury Balance</el-button>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="Member" min-width="120px"align="right">
        <template slot-scope="{row}">
    ...
    <el-dialog title="Debit Luxury Balance" width="60%"  top="30px" :visible.sync="dialogDebitBalanceVisible">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" icon="el-icon-finished" :loading="buttonLoading" @click="handleDebitBalance()">Debit Luxury Balance</el-button>
      </span>
    </el-dialog>
...
<script>
import { getLuxuryWalletDebitTransactions, debitLuxuryBalance } from "@/api/admin/wallet";
import { getMemberLuxuryBalance } from "@/api/admin/members";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getLuxuryWalletDebitTransactions(this.listQuery).then(response => {
    ...
    handleFromCheckMemberId(){
      if(this.temp.transfer_from){
        getMemberLuxuryBalance(this.temp.transfer_from).then((response) => {
          this.temp.transfer_from_name=response.data.name;
          this.temp.balance=response.data.member.luxury_wallet_balance;
        }).catch((error)=>{
    ...
    handleAddMemberGetBalance(){
       if(this.tempDebitBalance.member_id){
        getMemberLuxuryBalance(this.tempDebitBalance.member_id).then((response) => {
          this.tempDebitBalance.member_name=response.data.name;
          this.tempDebitBalance.balance=response.data.member.luxury_wallet_balance;    
        }).catch((error)=>{
    ...
    handleToCheckMemberId(){
      if(this.temp.transfer_to){
        getMemberLuxuryBalance(this.temp.transfer_to).then((response) => {
    ...
    handleDebitBalance(){
      this.$refs["formDebitBalance"].validate(valid => {
        ...
        if (valid) {
          ...
          debitLuxuryBalance(this.tempDebitBalance).then((response) => {
            this.getList();
