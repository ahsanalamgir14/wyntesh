<template>
  <div class="app-container">    
    ...
    <el-table
      ...
      <el-table-column label="Member" class-name="status-col" width="200px">
        <template slot-scope="{row}">
            {{ row.member.user.username }}
        </template>
      </el-table-column>

      <el-table-column label="Member Name" class-name="status-col" width="180px">
        <template slot-scope="{row}">
            {{ row.member.user.name }}
        </template>
      </el-table-column>
      <el-table-column label="Amount" class-name="status-col" width="180px">
        <template slot-scope="{row}">
            {{ row.amount }}
        </template>
      </el-table-column>
      <el-table-column label="Income wallet Balance" class-name="status-col" width="180px">
        <template slot-scope="{row}">
            {{ row.member.income_wallet_balance }}
        </template>
      </el-table-column>
      <el-table-column label="E-wallet Balance" class-name="status-col" width="180px">
        <template slot-scope="{row}">
            {{ row.member.wallet_balance }}
        </template>
      </el-table-column>
      <el-table-column label="Note" class-name="status-col" width="180px">
        <template slot-scope="{row}">
            {{ row.note }}
        </template>
      </el-table-column>
    ...
    <el-dialog title="Approve" width="60%" top="30px" :visible.sync="dialogApproveVisible">
      <el-form ref="formApprove" :rules="rules" :model="temp">
        <el-row :gutter="20">
          <el-col  :xs="24" :sm="24" :md="24" :lg="24" :xl="24" >
            <el-form-item label="Amount to E-wallet" prop="amount" label-width="120">
...
} from "@/api/admin/transfer-request";
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchTransferRequests(this.listQuery).then(response => {
    ...
    rejectRequest(row){
      this.$prompt('Enter note', 'Confirm Rejection', {
          ...
          rejectTransferRequest(data).then((res) => {          
            this.$notify({
                title: "Success",
                message: res.message,
                type: "success",
    ...
    approveRequest(){
      this.$refs["formApprove"].validate(valid => {
        ...
        if (valid) {
           this.buttonLoading=true;
          approveTransferRequest(this.temp).then((response) => {
            this.getList();
            this.resetTemp();
            this.buttonLoading=false;
