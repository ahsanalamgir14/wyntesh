<template>
  <div class="app-container">
    ...
    <el-dialog :title="textMap[dialogStatus]" width="60%" top="30px"  :visible.sync="dialogPaymentModeVisible">
      <el-form ref="paymentModeForm" :rules="rules" :model="temp" style="">
        <el-row>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogPaymentModeVisible = false">
          Cancel
        </el-button>
...
import {
  ...
  createPaymentMode,
  updatePaymentMode
} from "@/api/superadmin/payment-modes";
...
export default {
  ...
  data() {
    return {
      ...
      dialogPaymentModeVisible:false,
      dialogStatus: "",
      textMap: {
        update: "Edit",
        create: "Create"
  ...
  methods: {
    ...
    handleCreate() {
      ...
      this.dialogPaymentModeVisible = true;
      this.$nextTick(() => {
        this.$refs["paymentModeForm"].clearValidate();
      });
    },
    createData() {
      this.buttonLoading=true;
      this.$refs["paymentModeForm"].validate(valid => {
        if (valid) {
         
          createPaymentMode(this.temp).then((data) => {
            this.list.unshift(data.data);
            this.dialogPaymentModeVisible = false;
            this.$notify({
              title: "Success",
    ...
    updateData() {
      ...
      this.$refs["paymentModeForm"].validate(valid => {
        if (valid) {
          ...
          updatePaymentMode(tempData).then((data) => {
    ...
    deleteData(row) {
        deletePaymentMode(row.id).then((data) => {
            this.dialogPaymentModeVisible = false;
            this.$notify({
                title: "Success",
                message: data.message,
